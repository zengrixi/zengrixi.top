...
- [编译osgEarth](#编译osgearth)
  1. [编译curl](#编译curl-7653)
  2. [编译sqlite3](#编译sqlite3)
  3. [编译proj](#编译proj620)
  4. [编译GDAL](#编译gdal-321)
  5. [编译zlib](#编译zlib-1211)
- [结尾（附下载链接）](#结尾)
...
1. 使用CMake配置目标工程
   ...
   **（因为博主需要用[tif插件](https://gitlab.com/libtiff/libtiff)，所以额外加入了tif库。）**

   ![tif](tif.png)

2. 编译生成好的工程

   博主喜欢用命令行编译。（记得debug和release都编译一遍。）

   ```shell
   # 更改后面字符串为Debug则生成Debug版本
   msbuild ALL_BUILD.vcxproj /p:Configuration="Release"
   msbuild INSTALL.vcxproj /p:Configuration="Release"
   ```
   
   **（PS：INSTALL.vcxproj 生成的路径默认为C盘，可以通过`CMAKE_INSTALL_PREFIX`后的路径来修改。）**
...
（**PS：博主的是1929，可以参照[官网](https://docs.microsoft.com/en-us/cpp/preprocessor/predefined-macros?view=msvc-160&viewFallbackFrom=vs-2019)得到自己的版本号**。）
...
```shell
nmake /f makefile.vc MSVC_VER=1929 WIN64=1 DEBUG=0
...
```shell
nmake /f makefile.vc MSVC_VER=1929 WIN64=1 DEBUG=1
nmake /f makefile.vc devinstall
```
